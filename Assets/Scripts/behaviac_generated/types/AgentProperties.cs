...
namespace behaviac
{
	...
	public class BehaviorLoaderImplement : BehaviorLoader
	{
		...
		{
			AgentMeta.TotalSignature = 1585046185;
			...
			// NewAgent
			meta = new AgentMeta(2163288822);
			AgentMeta._AgentMetas_[1515050225] = meta;
			meta.RegisterMemberProperty(2436539256, new CMemberProperty<int>("age", delegate(Agent self, int value) { ((NewAgent)self).age = value; }, delegate(Agent self) { return ((NewAgent)self).age; }));
			meta.RegisterMemberProperty(3282915872, new CMemberProperty<string>("name", delegate(Agent self, string value) { ((NewAgent)self).name = value; }, delegate(Agent self) { return ((NewAgent)self).name; }));
			meta.RegisterMethod(1045109914, new CAgentStaticMethodVoid<string>(delegate(string param0) { NewAgent.LogMessage(param0); }));
			meta.RegisterMethod(3625801152, new CAgentMethodVoid(delegate(Agent self) { ((NewAgent)self).SayAge(); }));
			meta.RegisterMethod(3793808279, new CAgentMethodVoid(delegate(Agent self) { ((NewAgent)self).SayName(); }));
			meta.RegisterMethod(2521019022, new CMethod_behaviac_Agent_VectorAdd());
			meta.RegisterMethod(2306090221, new CMethod_behaviac_Agent_VectorClear());
			meta.RegisterMethod(3483755530, new CMethod_behaviac_Agent_VectorContains());
			meta.RegisterMethod(505785840, new CMethod_behaviac_Agent_VectorLength());
			...
			AgentMeta.Register<NewAgent>("NewAgent");
		...
		public override bool UnLoad()
		{
			...
			AgentMeta.UnRegister<NewAgent>("NewAgent");
			return true;
		}
	}
}
